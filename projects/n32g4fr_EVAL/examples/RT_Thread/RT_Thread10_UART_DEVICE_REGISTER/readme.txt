...
3. 使用说明
    ...
    描述Demo的测试步骤和现象 
         1. 编译后下载程序复位运行
         2. 本例程在main()里面创建两个线程，test0 线程和 test1 线程，test0 线程用于控制 D1 250ms闪烁，在test1线程中测试 UART 设备发送接收功能，
            通过串口工具接在 USART2 的 TX 和 RX 上，上电后会先通过 USART2 的 TX 发送数据，上位机软件通过串口工具发送数据到 USART2 的 RX 上，USART2 会将接收到的数据通过 TX 再发送出去
